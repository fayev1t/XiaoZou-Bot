...
- **🔄 Tick-Based AgentLoop Architecture**  
  Ticks are awakened to execute when events arrive or internal timers trigger. The loop operates through `Event Ingestion -> State Folding (Projection) -> LLM Code Generation -> Sequential Execution in Sandbox -> Domain Event/Terminal Closure`. On preflight failure it cools the current LLM endpoint and retries decide with the next model in the role group (up to 3 attempts, no validation-feedback rewrite). Crash or exception events close as `interrupted` / `uncertain` during startup recovery.
