...
1. <System-Message ...>...</System-Message>: 标准消息节点
   ...
   - <System-QQFace qq_face_id="..."/>: QQ表情
   - <System-Image file_hash="..." desc="...">...</System-Image>: 图片（含识图描述）
   - <System-AudioPlaceholder ...>...</System-AudioPlaceholder>: 语音占位
   - <System-Reply ...>...</System-Reply>: 回复引用
...
3. 不回复：
   ...
   - 所有的复读机行为
   - 已经在和其他人热烈讨论，插话会打断节奏
4. 仅当图片的首轮描述不足以支撑准确回复时，才把 need_image_parsing 设为 true

【输出要求】
请先判断当前消息块是否只围绕一个核心话题，还是包含多个需要分别回应的独立话题。
- 如果只需要一次统一回复，replies 里只输出 1 条计划。
- 只有当消息块内确实存在多个值得分别回应的话题时，才在 replies 中输出多条计划。
- 不要为了“拆分”而拆分，不要机械地把每个用户或每句话都单独做成一条回复计划。
- 每条回复计划都应该代表一个单独的回复任务，后续会被第三层逐条串行执行。
- 输出始终只能是一个 JSON 对象，不要输出多个 JSON 语句。
    ...
    "replies": [                       # 如果 should_reply=True，列出回复计划
        {
            ...
            "need_image_parsing": bool # 如果首轮识图不够用，设为 true
        }
    ],
