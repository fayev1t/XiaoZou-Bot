...
权限：required_permission=OWNER —— 改群名是全群可见的高曝光改动，须群主授意。

OneBot action：set_group_name(group_id, group_name)。注意 napcat 参数名是
``group_name``（不是 name）。
"""
...
_USAGE_PROMPT = load_sibling_md(__file__, "set_group_name.md")
    ...
    async def execute(self, arguments: dict, **context: Any) -> ToolOutcome:
        ...
        if not name or not isinstance(name, str) or not name.strip():
            return ToolOutcome.failure(
                "invalid_arguments", f"{self.name}.name is required"
            )
        ...
        if fail:
            return fail
        logger.info("[{}] group={} name={}", self.name, group_id, name)
        return ToolOutcome.success(group_id=group_id, group_name=name)
