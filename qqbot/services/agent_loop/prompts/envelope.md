...
<工具>send_messages 的特例：参数/结果不渲 JSON，改为逐气泡一行——
  ...
  原因 <文字>（仅失败）
  气泡内容按发送顺序排列，chat 气泡的段落按 text 原文 / @QQ / 回复#消息ID /
  <表情N> 拼接。回执三态：→#消息ID 送达（该气泡在 QQ 上的消息 ID）；
...
<程序>失败[ 查询 名[,名…]] kind [k=v …]
  原因 <文字>

  一拍程序的终态。查询名按实际执行顺序列出，不含参数与查询结果；effect 不列
  在这里，因为它们各自已有 `<工具>` 行。完成时只有程序显式 `return` 了数据
  才有结果行。没有查询、没有 return、也没有失败的空程序整行不出现。
  程序源码永不进入信封。失败 kind 除工具自身错误外，程序层常见取值：
...
<系统>kind [载荷]
  运行时发出的一条提示，载荷按 kind 与正文字面理解。kind 常见取值：
    ...
    llm_invalid_output    此前某次输出被校验拒绝，载荷含原因与尝试序号。
    bot_role_observed     系统观测到本账号的群角色。
    napcat_unknown_event  平台推送了运行时没有解析器的事件类型，原始报文
                          原样附上。
...
<校验拒绝>attempt=N kind=错误类 [line=N [column=N]]: 正文
  <rejected-program>
    被拒源码（逐行缩进，动态结构字符已转义）
  </rejected-program>
  当同一拍的上一次程序被静态预检拒绝时出现，是信封的最后一个行块。行列按
  模型原始源码计；被拒源码只供本次同拍修复，不进入后续拍的时间线。
