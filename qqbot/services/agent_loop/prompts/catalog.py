...
现在 Planner 的人格、系统事实、行动纪律与输出契约按模型的
阅读顺序写在同一张页上（你是谁 → 你所处的系统 → {{envelope}} →
{{tools_usage}} → 你需要做什么 → 你的输出）。2026-08-01 维护者定稿：输入
信封字典与工具说明紧跟系统事实——先给定义再给纪律——行为规范随后，
输出契约收尾、离真实输入最近。角色卡仍在页首、仍是
第二人称——那就是她自己，旧的「这个qq号背后的人格是…」第三方框定引导语随
Replyer 退役，**不要再
加回来**（那句话存在的唯一理由是把卡片框成对下游角色的描述，而下游角色已不存在）。
...
资产分工：
  ...
  - `planner.md`  Planner 的行为约定，一页读完：角色卡（页首）→系统运行
                  事实与行动纪律（念头≠动作 / 跨拍靠任务 / 一批工具不要
                  重拨）及两步发言流程（reply 等待 → completed 唤醒 →
                  send_messages 落笔）→信封与工具规范→职责与表达倾向→
                  决策 JSON 输出契约。
...
装配为什么这么分（历史故障，改动前先读）：
  - **角色卡只有一份真相源（`planner.md` 页首那一段）。** 历史上 `planner.md`
