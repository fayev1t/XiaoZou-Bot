"""Contract tests for WholeBanTool（全员禁言开关）。
...
class _StubBot:
    ...
    async def set_group_whole_ban(self, **kwargs: Any) -> dict:
        self.calls.append(("set_group_whole_ban", kwargs))
        if self._raise is not None:
            raise self._raise
        return {}


class WholeBanToolTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        bot_registry.clear()
    ...
    async def test_whole_ban_happy_path(self) -> None:
        bot = _StubBot()
        bot_registry.register(bot)
        outcome = await WholeBanTool().run(
            {"enable": False},
            scope_key="group:100",
            **_OK_CTX,
        )
        ...
        self.assertEqual(method, "set_group_whole_ban")
        self.assertEqual(kwargs["group_id"], 100)
        self.assertFalse(kwargs["enable"])
        self.assertTrue(outcome.ok)
        self.assertEqual(outcome.result["group_id"], 100)
        self.assertFalse(outcome.result["enable"])

    async def test_enable_defaults_true(self) -> None:
        bot = _StubBot()
        bot_registry.register(bot)
        outcome = await WholeBanTool().run({}, scope_key="group:100", **_OK_CTX)
    ...
    async def test_non_group_scope_raises(self) -> None:
        ...
        outcome = await WholeBanTool().run(
            {"enable": True}, scope_key="system", **_OK_CTX
        )
    ...
    async def test_no_bot_raises(self) -> None:
        bot_registry.clear()
        outcome = await WholeBanTool().run(
            {"enable": True}, scope_key="group:100", **_OK_CTX
        )
        self.assertFalse(outcome.ok)
    ...
    async def test_napcat_failure_is_upstream_action_failed(self) -> None:
        ...
        self.assertEqual(outcome.extra["action"], "set_group_whole_ban")
    ...
    def test_metadata(self) -> None:
        self.assertEqual(WholeBanTool.name, "whole_ban")
        self.assertEqual(WholeBanTool.allowed_scopes, ("group",))
        self.assertEqual(WholeBanTool.required_permission, PermissionTier.OWNER)
        self.assertEqual(WholeBanTool.required_bot_role, "admin")

    def test_usage_md_loaded(self) -> None:
        self.assertIn("set_group_whole_ban", WholeBanTool.usage_prompt)
