...
build_default_registry()：30 个内置工具里它当前注册 16 个，另 14 个（ban / poke /
...
def _registry_of_all_builtin_tools() -> ToolRegistry:
    ...
    build_default_registry() 当前实际注册了哪些工具**无关**：默认注册表留 16 个，
    但另外 14 个下架工具的权限闸门仍须正确。故遍历 tools.__all__ 里的全部
