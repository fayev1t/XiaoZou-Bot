...
<table border="0">
  <tr>
    <td style="border: none; vertical-align: middle;">
      ...
      • <b>两步发言的克制</b>：决定开口不等于立刻说话——先存下对局势的解析并等待，到点再结合等待期间的新消息决定说什么、还说不说。<br>
...
- **两步发言（Deliberate Speech）**：说话不是一个动作。第一步 `reply` 只存下对局势的解析——谁在跟谁说、有哪几条话题线、什么还悬着——并开始一段短等待；等待到点，这份解析原样回到时间线并唤醒 Planner，那一拍它重读**等待期间新到的消息**，再决定是照原意开口、改口，还是判断时机已过继续沉默。落笔的字句永远是在最新上下文里选的，不是等待开始时就写死的。
...
所有能力经统一 `Tool` 接口接入（`qqbot/services/agent_loop/tools/`，每个工具自带同名 `.md` 说明文档）。下表是**当前实际注册**的 15 个工具：

| 分类 | 工具 |
|---|---|
| 表达 | `reply`（存解析并等待） `send_messages`（多条气泡，含表情包） `meme_collection`（表情包收藏与描述） |
...
> 仓库里另有 15 个已实现但**暂未启用**的工具（`ban` `poke` `recall` `set_card` `set_essence` 等），代码、文档与契约测试都在，逐个达标后再单独放行。
...
- **人设卡自定义 (`prompts/planner.md`)**：编辑 `qqbot/services/agent_loop/prompts/planner.md` 的 `# 你是谁` 段调整人格卡（render 时才读盘，改完即生效，无需重启）。
